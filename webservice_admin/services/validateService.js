...
const validateUserService = async (user_service) => {
    try {
        ...
        return await db.collection(mongoCollections.TOKENS_SERVICE).findOne(filter);
    } catch(error) {
        logger.error(error);
        return false;
    }
...
const updateTokenService = async (user_service, password, system) => {
    let result = {};
    try {
        const db = await mongodb.getDb();
        let filter = {
        ...
        system ? filter.sistema = system : null;
        ...
        let event_history = {
            event: 'login_update_service',
            collection: mongoCollections.TOKENS_SERVICE,
            system: user.sistema,
            token: jwtToken,
            date: new Date()
        }
...
const getUserSystem = async (sistema) => {
    try {
        ...
        let user = await db.collection(mongoCollections.TOKENS_SERVICE).findOne(filter);
        return {user: user.user_service, sistema: user.sistema};
    } catch(error) {
        logger.error(error);
        return {user: false};
    }
