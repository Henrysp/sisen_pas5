...
const getCacheBox = async (req, res, next) => {
  ...
  let jobAreaTypes = await catalogService.getCatalogByType(appConstants.CATALOG_TYPE_JOB_AREA);
  ...
  let data = {
    acreditationTypes: acreditationTypes.catalogs,
    jobAreaTypes: jobAreaTypes.catalogs
  };
...
const updateCatalog = async (req, res, next) => {
  const { id, type, value, valueold } = req.body;
  ...
  let data = await catalogService.updateCatalog(id, type, value, valueold, usuarioRegistro);
  return res.json(data);
}
...
const listCatalog = async (req, res, next) => {
  ...
  let data = await catalogService.listCatalog(type);
  return res.json(data);
}
...
const getNextCode = async (req, res, next)=>{
  const { type } = req.query;
  let data = await catalogService.getNextCode(type);
  return res.json(data);
}

module.exports = { getCacheBox, getCacheSendNotification, paginateCatalog, createCatalog, updateCatalog, removeCatalog, getTypes, listCatalog, getNextCode };