...
import {
  listDoc,
  listDocRep,
} from 'src/app/pages/main/admin/user/edit-citizen/store/casilla-ciudadano.model';
...
@Component({
  ...
  styleUrls: ['./edit-citizen.component.scss'],
...
export class EditCitizenComponent implements OnInit {
  ...
  direccion: FormControl = new FormControl({ value: '' }, [Validators.required, Validators.minLength(9)]);
  correo: FormControl = new FormControl({ value: '' }, [Validators.required, Validators.pattern('[a-zA-Z0-9.+-_]{1,}@[a-zA-Z.-]{2,}[.]{1}[a-zA-Z]{2,}')]);
  organizacion: FormControl = new FormControl({value: '' },[]);
  ...
  private loadExtranjeria = (doc: string) => {
    return new Promise<boolean>((resolve) => {
      resolve(false);
    });
  };
  private loadDocument = async (type: string) => {
    var tipo = '';
    var doc = '';
    this.load = true;
    if (type == 'general') {
    ...
    switch (tipo) {
      case 'ce':
        response = await this.loadExtranjeria(doc);
        message =
          'El CE ' +
          doc +
        ...
        break;
      case 'dni':
        response = await this.loadReniec(doc);
        message =
  ...
  private loadSubmit = () => {
    ...
    if (!this.user) {
      ...
      request = {
        ...
        user_address: this.direccion.value,
        user_acreditation_type: this.form.controls['acreditacion'].value,
        user_organization_name: this.organizacion.value,
        //casilla
        box_doc_type: this.user.doc_type,
        box_doc: this.user.doc,
        box_organization_name: this.organizacion.value,
        box_email: this.correo.value,
        box_address: this.direccion.value,
